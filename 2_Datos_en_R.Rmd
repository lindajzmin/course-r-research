...
- Importar datos
   ...
   * Importar data sets desde la web
   * Otros archivos
...
.pull-left[
```{r datos1, echo=FALSE, out.width='100%', fig.align='center'}
...
.pull-right[
```{r datos2, echo=FALSE, out.width='100%', fig.align='center'}
knitr::include_graphics("img/attributes.png")
```
]

.footnote[ [Understanding the Data Science Lifecycle](https://www.sudeep.co/data-science/2018/02/09/Understanding-the-Data-Science-Lifecycle.html) || [Your Guide to Data Quality Management](https://www.scnsoft.com/blog/guide-to-data-quality-management)]

---

# `r fontawesome::fa("database")` Proceso del *tidyverse*
...
# `r fontawesome::fa("dumbbell")` Ejercicio 1: profesores.xlsx

### 1. Cargar el archivo `profesores.xlsx`
```{r ejer1, echo=TRUE}
...
# `r fontawesome::fa("table")` Tibbles

.pull-left[

.center[
<img src="img/tibble.png" width="200px">
]

Los `tibble` son data.frames, pero modifican algunas características antiguas para hacernos la vida más fácil. La mayoría de las veces usaremos el término tibble y data.frame de manera indistinta. 
...
La mayoría de los paquetes de R suelen usar data.frames clásicos, así que si deseas trabajar con tibbles (que es más recomendable) puedes convertir un data.frame en un tibble, con la función `as_tibble()`.

```{r tb2, echo=TRUE}
tbl_pacientes2 <- as_tibble(pacientes2)
```
]
...
Vamos a visualizar la diferencia entre la estructura de un data.frame y de un tibble con `str()`:

.pull-left[
**Data.frame**
```{r tb1, echo=TRUE}
str(pacientes2)           # muestra estructura de data.frame
```
]
...
**Tibble**
```{r tb3, echo=TRUE}
str(tbl_pacientes2)       # muestra estructura de tibble
```
]

---

```{r tid2, echo=FALSE, out.width='90%', fig.align='center', fig.cap="Figura 1. Diferencias entre estructuras de datos"}
knitr::include_graphics("img/estructuras.png")
```

---

# `r fontawesome::fa("dumbbell")` Ejercicio 2: profesores.xlsx

### 1. Indique la estructura que tiene el data set `profesores.xlsx`.
...
# `r fontawesome::fa("dumbbell")` Ejercicio 3: profesores.xlsx

### 1. Prepare el data set para el análisis verificando que el tipo de dato de cada variable sea el adecuado.

**Fecha**
```{r ejer2, echo=TRUE}
profesores$Fecha <- as_date(profesores$Fecha)
class(profesores$Fecha)
```

**Tiempo impartiendo docencia**
```{r ejer3, echo=TRUE}
...
**Atención a la diversidad es necesaria**
...
**Atención a la diversidad se imparte en cualquier asignatura**
...
**Leyes de educación para alumnos con necesidades educativas especiales?**
...
**Ratio de alumnos con necesidades educativas especiales**
...
**Asistente de clases a tiempo completo**
...
**Instrucción suficiente para atender alumnos con necesidades educativas especiales**
...
# `r fontawesome::fa("pen")` Práctica 2


1. Carga el archivo `BasePrograma.xlsx`

2. Verifique la estructura que tiene el data set.

3. Indique el número de observaciones y de variables del data set.

4. Prepare el data set para el análisis verificando que el tipo de dato de cada variable sea el adecuado. Corrija los tipos de datos que considere necesarios.
