...
Hay docenas de capas de **Stats** especificadas con un `stat_function`. La mayoría de los geoms y las estadísticas vienen en pares que casi siempre se usan en conjunto.
...
.pull-left[
Empecemos con un gráfico de barras que aunque parece simple revela algo sutil sobre los gráficos.

```{r te1, echo=TRUE, eval=FALSE}
...
.pull-left[
Puedes mostrar un gráfico de barras de proporciones, en lugar de un recuento:

```{r te4, echo=TRUE, eval=FALSE}
diamantes %>% ggplot(aes(x = corte, 
                         y=stat(prop), 
                         group=1)) +
    stat_count()
...
.pull-right[
```{r te5, echo=FALSE, eval=TRUE, warning=FALSE, message=FALSE}
diamantes %>% ggplot(aes(x = corte, 
                         y=stat(prop), 
                         group=1)) +
    stat_count()
